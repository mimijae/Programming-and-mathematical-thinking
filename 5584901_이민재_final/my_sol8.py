            ...
            if self.menu_on:  # 메뉴 화면일 때 이벤트 처리
                ...
                if event.type == pygame.KEYDOWN:  # 키가 눌렸을 때 처리
                    if event.key == pygame.K_LEFT:  # 좌측 이동
                        ...
                        # 미사일 3개를 생성하고 발사
                        missile1 = Missile(self.fighter.rect.centerx-20, self.fighter.rect.y-15, 10)
                        missile2 = Missile(self.fighter.rect.centerx-10, self.fighter.rect.y-10, 10)
                        missile3 = Missile(self.fighter.rect.centerx, self.fighter.rect.y, 10)
                        missile4 = Missile(self.fighter.rect.centerx+10, self.fighter.rect.y+10, 10)
                        missile5 = Missile(self.fighter.rect.centerx+20, self.fighter.rect.y+15, 10)
                        missile1.launch()
                        missile2.launch()
                        missile3.launch()
                        missile4.launch()
                        missile5.launch()
                        self.missiles.add(missile1, missile2, missile3, missile4, missile5)  # 미사일 그룹에 추가
