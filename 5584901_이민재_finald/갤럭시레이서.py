...
def move_starship(scrn, key):  # 플레이어 기체 이동
    ...
    ss_d = 0  # 기체 방향 초기화 (0: 정면)
    if key[K_UP] == 1:  # 위쪽 화살표 키 입력 처리
        ss_y = ss_y - 20  # 위로 이동
        if ss_y < 80:  # 화면 위쪽 경계 처리
            ss_y = 80
    if key[K_DOWN] == 1:  # 아래쪽 화살표 키 입력 처리
        ss_y = ss_y + 20  # 아래로 이동
        if ss_y > 640:  # 화면 아래쪽 경계 처리
            ss_y = 640
    if key[K_LEFT] == 1:  # 왼쪽 화살표 키 입력 처리
        ss_d = 1  # 기체 방향을 왼쪽으로 설정
        ss_x = ss_x - 20  # 왼쪽으로 이동
        if ss_x < 40:  # 화면 왼쪽 경계 처리
            ss_x = 40
    if key[K_RIGHT] == 1:  # 오른쪽 화살표 키 입력 처리
        ss_d = 2  # 기체 방향을 오른쪽으로 설정
        ss_x = ss_x + 20  # 오른쪽으로 이동
        if ss_x > 920:  # 화면 오른쪽 경계 처리
            ss_x = 920

    key_spc = (key_spc + 1) * key[K_SPACE]  # 스페이스바 누를 때 미사일 발사 (주기적으로 발사)
    if key_spc % 5 == 1:  # 5프레임마다 미사일 발사
        set_missile(0)  # 기본 미사일 발사
        # se_shot.play()  # 발사 효과음

    key_z = (key_z + 1) * key[K_z]  # Z키를 누르면 특수 미사일 발사
    if key_z == 1 and ss_shield > 10:  # 실드가 충분할 때만 특수 미사일 사용
        set_missile(10)  # 특수 미사일 발사
        ss_shield = ss_shield - 10  # 실드 감소
        # se_barrage.play()  # 특수 발사 효과음

    if ss_muteki % 2 == 0:  # 무적 상태에서 깜빡임 효과 처리
        scrn.blit(img_sship[3], [ss_x - 8, ss_y + 40 + (tmr % 3) * 2])  # 보조 그래픽 출력
        scrn.blit(img_sship[ss_d], [ss_x - 37, ss_y - 48])  # 방향에 맞는 플레이어 이미지 출력

    if ss_muteki > 0:  # 무적 상태 유지 시간 감소
        ss_muteki = ss_muteki - 1
        return

    elif idx == 1:  # 게임 진행 상태 (충돌 체크)
        for i in range(ENEMY_MAX):  # 모든 적 기체와 충돌 검사
            if emy_f[i] == True:  # 적 기체가 활성화된 상태
                w = img_enemy[emy_type[i]].get_width()  # 적 기체 너비
                h = img_enemy[emy_type[i]].get_height()  # 적 기체 높이
                r = int((w + h) / 4 + (74 + 96) / 4)  # 충돌 범위 설정
                if get_dis(emy_x[i], emy_y[i], ss_x, ss_y) < r * r:  # 거리 계산으로 충돌 여부 확인
                    set_effect(ss_x, ss_y)  # 폭발 효과 출력
                    ss_shield = ss_shield - 10  # 실드 감소
                    if ss_shield <= 0:  # 실드가 0 이하일 경우
                        ss_shield = 0
                        idx = 2  # 게임 오버 상태로 전환
                        tmr = 0  # 타이머 초기화
                    if ss_muteki == 0:  # 무적 상태가 아닐 때
                        ss_muteki = 60  # 60프레임 동안 무적 상태 설정
                        # se_damage.play()  # 피해 효과음
                    if emy_type[i] < EMY_BOSS:  # 보스가 아닌 경우 적 제거
                        emy_f[i] = False

def set_missile(typ):  # 플레이어 기체 발사 탄환 설정
    global msl_no  # 현재 사용할 미사일 번호를 나타내는 전역 변수
    if typ == 0:  # 단발 모드 (단일 탄환 발사)
        msl_f[msl_no] = True  # 해당 미사일 활성화 (True로 설정)
        msl_x[msl_no] = ss_x  # 미사일의 x 좌표를 플레이어 기체 위치로 설정
        msl_y[msl_no] = ss_y - 50  # 미사일의 y 좌표를 기체보다 위로 설정 (발사 위치)
        msl_a[msl_no] = 270  # 미사일 발사 각도를 270도(위쪽 방향)로 설정
        msl_no = (msl_no + 1) % MISSILE_MAX  # 미사일 번호를 순환하도록 설정 (최대 미사일 개수 유지)
    if typ == 10:  # 탄막 모드 (다수의 탄환 발사)
        for a in range(160, 390, 10):  # 160도에서 380도까지 10도 간격으로 발사 (확산형 패턴)
            msl_f[msl_no] = True  # 해당 미사일 활성화
            msl_x[msl_no] = ss_x  # 미사일의 x 좌표를 플레이어 기체 위치로 설정
            msl_y[msl_no] = ss_y - 50  # 미사일의 y 좌표를 기체보다 위로 설정
            msl_a[msl_no] = a  # 미사일 발사 각도를 a 값으로 설정 (각도에 따라 방향 결정)
            msl_no = (msl_no + 1) % MISSILE_MAX  # 미사일 번호를 순환하도록 설정
